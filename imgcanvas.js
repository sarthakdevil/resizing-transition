var canvas = document.querySelector('canvas');
var c = canvas.getContext('2d');
canvas.width = window.innerWidth;
canvas.height = window.innerHeight;

var mouse = {
  x: undefined,
  y: undefined
};

addEventListener('mousemove', function(event) {
  mouse.x = event.x;
  mouse.y = event.y;
});

var circles = [];

function Circle(x, y, dx, dy, color) {
  this.x = x;
  this.y = y;
  this.dx = dx;
  this.dy = dy;
  this.color = color;
  this.radius = 30;

  this.draw = function() {
    c.beginPath();
    c.arc(this.x, this.y, this.radius, 0, Math.PI * 2, false);
    c.strokeStyle = this.color;
    c.stroke();
    c.fillStyle = this.color;
    c.fill();
  };

  this.update = function() {
    this.x += this.dx;
    this.y += this.dy;

    // Reverse direction if circles hit the edges of the canvas
    if (this.x < this.radius || this.x > canvas.width - this.radius) {
      this.dx = -this.dx;
    }
    if (this.y < this.radius || this.y > canvas.height - this.radius) {
      this.dy = -this.dy;
    }

    // Calculate distance between circle and mouse pointer
    var distanceX = Math.abs(this.x - mouse.x);
    var distanceY = Math.abs(this.y - mouse.y);
    var distance = Math.sqrt(distanceX * distanceX + distanceY * distanceY);

    // Increase circle radius if mouse pointer is close
    if (distance < 100 && this.radius < 50) {
      this.radius += 2;
    } else if (distance >= 100 && this.radius > 30) {
      this.radius -= 2;
    }

    this.draw();
  };
}

for (var i = 0; i < 200; i++) {
  var x = Math.random() * (canvas.width - 60) + 30;
  var y = Math.random() * (canvas.height - 60) + 30;
  var dx = (Math.random() - 0.5) * 5;
  var dy = (Math.random() - 0.5) * 5;
  var color =
    'rgb(' +
    Math.floor(Math.random() * 256) +
    ', ' +
    Math.floor(Math.random() * 256) +
    ', ' +
    Math.floor(Math.random() * 256) +
    ')';

  var circle = new Circle(x, y, dx, dy, color);
  circles.push(circle);
}

function animate() {
  requestAnimationFrame(animate);
  c.clearRect(0, 0, canvas.width, canvas.height);

  for (var i = 0; i < circles.length; i++) {
    circles[i].update();
  }

  c.fillStyle = 'cyan';
  c.globalAlpha = 0.5;
  c.font = 'bold 50px sans-serif';
  c.
  c.fillText('Avinash Edu Hub', 600, 350);
  c.font = 'bold 25px comic-sans';
  c.fillText('', 600, 400);
}

animate();
